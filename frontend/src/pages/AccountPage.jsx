...
export default function AccountPage() {
  ...
  // Fetch saved words if logged in
  useEffect(() => {
    if (!token) return;
    ...
    const fetchWords = async () => {
      setLoading(true);
      try {
        const res = await fetch("https://bd39183c5bcc.ngrok-free.app/words/display", {
          headers: {
        ...
        if (data?.words) {
          // Enrich words with translations + examples
  ...
  // Clean up word text
  const cleanWord = (w) => {
    return w.replace(/[^a-zA-ZÀ-ÿ'-]/g, "").trim();
  };

  // Fetch translation + example from MyMemory API
  const fetchWordInfo = async (word) => {
    try {
      ...
      // Fake example sentence using word + translation (can be replaced with better API)
      const example = `Je vois "${word}" dans une phrase. → "${translation}"`;
